...
export default function SearchPage() {
  ...
  const fetchHotels = async () => {
    try {
      ...
      // Filter only active hotels - DON'T filter by images
      const activeHotels = hotelsData.filter((h: Hotel) => h.isActive !== false);
      
      console.log('Active hotels:', activeHotels.length, activeHotels.map((h: Hotel) => h.name)); // DEBUG
        ...
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex flex-col md:flex-row gap-4 items-center">
            ...
            <div className="flex gap-2">
              <button className="px-4 py-2 bg-nomad-dark border border-nomad-border rounded-lg text-sm">
                Check In
              </button>
              <button className="px-4 py-2 bg-nomad-dark border border-nomad-border rounded-lg text-sm">
                Check Out
        ...
        <div className="flex gap-8">
          ...
          <div className="hidden lg:block w-64 flex-shrink-0">
            <div className="bg-nomad-card rounded-xl border border-nomad-border p-6">
              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {['hotel', 'apartment', 'hostel', 'all'].map((type) => (
                    <button key={type} className="px-3 py-1.5 bg-nomad-dark border border-nomad-border rounded-lg text-xs capitalize">
                      {type}
                    </button>
                  ))}
                </div>
              ...
              <div className="mb-6">
                ...
                <div className="flex gap-2">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button key={star} className="px-3 py-1.5 bg-nomad-dark border border-nomad-border rounded-lg text-xs">
                      {star}+
                    </button>
                  ))}
                </div>
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between mb-6">
              ...
              <select className="px-4 py-2 bg-nomad-card border border-nomad-border rounded-lg text-sm">
                ...
                {filteredHotels.map((hotel) => (
                  <div key={hotel.id} className="bg-nomad-card rounded-2xl border border-nomad-border overflow-hidden hover:border-crypto-green/30 transition-colors">
                    <div className="flex flex-col md:flex-row">
                      {/* Image - WITH FALLBACK */}
                      <div className="md:w-72 h-48 md:h-auto relative bg-nomad-dark">
                        <img 
